    ...
    const fetchData = async () => {
      if (customerId && visible) {
        try {
          ...
          // Fetch customer data và maCty options song song
          const [customerResponse, ] = await Promise.all([
            getCustomerById(customerId),
            fetchMaCtyOptions()
          ]);
          
          setInitialData(customerResponse);
          setSelectedItems(customerResponse.items || []);
          
          form.setFieldsValue({
            ...customerResponse,
            maCty: customerResponse.maCty || []
          });
        } catch (error) {
      ...
      >
        ...
        <Card size="small" title="Danh sách mặt hàng">
          ...
          {selectedItems.length > 0 && (
            <Form.Item label="Các mặt hàng hiện có">
              <div style={{ 
